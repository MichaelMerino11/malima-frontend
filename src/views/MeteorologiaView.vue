  ...
  <v-container fluid class="meteorologia-page pa-4 pa-md-6">
    ...
    >
      ...
      <div class="risk-alert__content">
        ...
        <v-chip size="small" color="warning" variant="flat"> Revisar invernaderos </v-chip>
      </div>
    </v-alert>

    <div v-if="!datos && !store.cargando" class="empty-state">
      ...
      <v-card rounded="xl" elevation="0" class="weather-overview mb-5">
        <div class="weather-overview__main">
          ...
          <div class="current-weather__info">
            <div>
              <span>Condición</span>
              <strong>{{ estadoClima.texto }}</strong>
            </div>

            <div>
              <span>Zona</span>
              <strong>{{ zonaActualNombre }}</strong>
            </div>
              ...
              >
                ...
                <template #item.temperatura="{ item }">
                  <div class="table-value">
                    <v-icon size="15" color="error"> mdi-thermometer </v-icon>
                ...
                <template #item.humedad="{ item }">
                  <div class="table-value">
                    <v-icon size="15" color="info"> mdi-water-percent </v-icon>
                ...
                <template #item.velocidad_viento="{ item }">
                  <div class="table-value">
                    <v-icon size="15" color="warning"> mdi-weather-windy </v-icon>

                    {{ item.velocidad_viento }}
                    km/h
                  </div>
                </template>
                ...
                <template #item.probabilidad_lluvia="{ item }">
                  <v-chip
                    size="x-small"
                    :color="
                      item.probabilidad_lluvia > 60
                        ? 'error'
                        : item.probabilidad_lluvia > 35
                          ? 'warning'
                          : 'success'
                    "
                    variant="tonal"
...
const probabilidadLluvia = computed(() => {
  return Number(datos.value?.probabilidad_lluvia ?? 0)
})
...
const riesgoClimatico = computed(() => {
  ...
  return probabilidadLluvia.value > 60 || Number(datos.value.velocidad_viento ?? 0) > 40
})
...
const textoAlerta = computed(() => {
  ...
  const viento = Number(datos.value.velocidad_viento ?? 0) > 40

  if (lluvia && viento) {
    return (
      `Se detectó ${probabilidadLluvia.value}% de probabilidad de lluvia ` +
      `y viento de ${datos.value.velocidad_viento} km/h. ` +
      'Se recomienda cerrar los invernaderos.'
    )
  }
  ...
  return (
    `La velocidad del viento alcanzó ${datos.value.velocidad_viento} km/h. ` +
    'Se recomienda cerrar los invernaderos.'
  )
})
...
const estadoClima = computed(() => {
  ...
  const v = datos.value.velocidad_viento ?? 0

  const r = datos.value.radiacion_solar ?? 0

  if (p > 60) {
    return {
      texto: 'Lluvia inminente',
      color: 'error',
...
const recomendacion = computed(() => {
  ...
  const p = Number(datos.value.probabilidad_lluvia ?? 0)

  const v = Number(datos.value.velocidad_viento ?? 0)
  ...
  if (p > 60 || v > 40) {
    return {
      ...
      texto:
        ...
        'Se recomienda realizar el cierre preventivo de los galpones.',
      color: 'error',
  ...
  return {
    ...
    texto: 'Las condiciones meteorológicas son favorables y no se detectan riesgos inmediatos.',
    color: 'success',
...
const condiciones = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      detalle: vientoFuerte ? 'Se recomienda cerrar galpones' : 'Velocidad segura',
...
const cargar = async () => {
  ...
  try {
    await Promise.all([store.cargarUltimoDato(zonaSeleccionada.value), cargarHistorial()])
